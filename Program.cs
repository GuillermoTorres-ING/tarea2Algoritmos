...
class Program
{
    ...
    static void MedirTiempo(Action action, string nombre) // ejecuta la acción mientras mide el tiempo
    {
        ...
        var memoryBefore = GC.GetTotalMemory(true); // Captura la memoria antes de ejecutar la acción
        reloj.Start();
        action();
        reloj.Stop();
        var memoryAfter = GC.GetTotalMemory(true); // Captura la memoria después de ejecutar la acción
        var memoryUsed = memoryAfter - memoryBefore; // Calcula la memoria utilizada
        Console.WriteLine($"Tiempo ({nombre}): {reloj.Elapsed.TotalMilliseconds} Milisegundos");
        Console.WriteLine($"Uso de memoria ({nombre}): {memoryUsed} KB");
    }
